-- =============================================================================
-- RPC: crear_denuncia
-- Crea (o reutiliza) el acusado y registra la denuncia + evidencias de forma
-- atómica, devolviendo el código de seguimiento. SECURITY DEFINER permite
-- encontrar-o-crear el acusado aunque aún no esté publicado, pero SIEMPRE
-- fuerza estado PENDIENTE: nada se publica sin moderación.
-- =============================================================================
create or replace function crear_denuncia(
  p_acusado     jsonb,   -- {cedula_prefix, cedula, nombres, apellidos, cargo, institucion, estado, municipio}
  p_tipo        tipo_delito,
  p_descripcion text,
  p_origen      origen_denuncia default 'TESTIMONIO',
  p_ocurrido_en date default null,
  p_evidencias  jsonb default '[]'::jsonb  -- [{tipo, url, nombre}]
)
returns text
language plpgsql
security definer
...
begin
  ...
  -- Reutiliza acusado por cédula si se proporcionó
  ...
  insert into denuncias (acusado_id, tipo, descripcion, origen, ocurrido_en, estado)
  values (v_acusado_id, p_tipo, trim(p_descripcion), coalesce(p_origen, 'TESTIMONIO'), p_ocurrido_en, 'PENDIENTE')
...
grant execute on function crear_denuncia(jsonb, tipo_delito, text, origen_denuncia, date, jsonb) to anon, authenticated;
